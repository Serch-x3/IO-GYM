...
--TRIGGERS
DELIMITER $$
CREATE OR REPLACE TRIGGER DeleteClass BEFORE DELETE ON GYMCLASSES FOR EACH ROW
    BEGIN
...
CREATE OR REPLACE TRIGGER DeleteTrainer BEFORE DELETE ON TRAINNERS FOR EACH ROW
    BEGIN
        UPDATE GROUPS SET trainer_id = NULL WHERE trainer_id = old.trainer_id;
        DELETE FROM TRAINNERS_ATTENDANCES WHERE TRAINNERS_ATTENDANCES.trainer_id = old.trainer_id;
    END$$
