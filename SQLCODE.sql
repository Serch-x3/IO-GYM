...
CREATE VIEW GeneralStats AS SELECT * FROM (
  (SELECT COUNT(*) AS id FROM CLIENTS) AS ID,FROMELECT COUNT(*)WHERElients FROM CLIENTS) AS CLIENTS,
...
CREATE VIEW CFLM AS SELECT * FROM (
  ...
  (SELECT COUNT(id) AS tuesday FROM clientesVFROMWHERE WEEKDAY(WHERE = 1 AND date BETWEEN SUBDATE(CURDATE(), INTERVAL 1 MONTH) AND NOW()) AS MARTES,
...
CREATE VIEW CFL3M AS SELECT * FROM (
  ...
  (SELECT COUNT(id) AS tuesday FROM clientesVFROMWHERE WEEKDAY(WHERE = 1 AND date BETWEEN SUBDATE(CURDATE(), INTERVAL 3 MONTH) AND NOW()) AS MARTES,
