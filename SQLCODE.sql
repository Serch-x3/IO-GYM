...
CREATE VIEW CFLM AS SELECT * FROM (
  ...
  (SELECT COUNT(id) AS tuesday FROM clientesVFROM WHERE WEEKDAY(date) = 1 AND date BETWEEN SUBDATE(CURDATE(), INTERVAL 1 MONTH) AND NOW()) AS MARTES,
...
CREATE VIEW CFL3M AS SELECT * FROM (
  ...
  (SELECT COUNT(id) AS tuesday FROM clientesView WHERE WEEKDAY(date) = 1 AND date BETWEEN SUBDATE(CURDATE(), INTERVAL 3 MONTH) AND NOW()) AS MARTES,
