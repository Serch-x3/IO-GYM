...
CREATE OR REPLACE VIEW clientAttendanceView AS SELECT row_number() OVER () AS id, CLIENTS.client_name AS client_name, CLIENTS.client_surname AS client_surname, CLIENT_ATTENDANCES.date AS date FROM CLIENTS, CLIENT_ATTENDANCES WHERE CLIENTS.client_id=CLIENT_ATTENDANCES.client_id order by date DESC;


--for Trainer Attendances
CREATE OR REPLACE VIEW trainerAttendanceView AS SELECT row_number() OVER () AS id, TRAINNERS.trainer_name AS trainer_name, TRAINNERS.trainer_surname AS trainer_surname, TRAINNERS_ATTENDANCES.date AS register_date, TRAINNERS_ATTENDANCES.description AS description FROM TRAINNERS, TRAINNERS_ATTENDANCES WHERE TRAINNERS.trainer_id=TRAINNERS_ATTENDANCES.trainer_id ORDER BY register_date DESC;
